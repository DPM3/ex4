...
int PathFinfBestFS::distanceFromEnd(GraphPoint p) {
    return 10 * (abs(p.x() - m_end.x()) + abs(p.y() - m_end.y())) -
            6 * std::min(abs(p.x() - m_end.x()), abs(p.y() - m_end.y()));
}

std::string PathFindBestFS::solve() const{
    ...
    while (!q.empty()) {
        ...
        for (std::pair pair : q) {
            if (std::pair.get<1>(pair) < std::pair.get<1>(current)) {
